...
5. **피처 수를 줄였는데도 발생하는 이유는?** (19개 피처에서도 동일한 문제)

---

## 질문 2: 선택된 피처 재학습 후 성능 하락 분석 ⚠️ 긴급
...
- **재학습 결과: 성능 하락 확인**
  ...
  - Best model: HistGradientBoostingClassifier (Stacking이 아닌 단일 모델)

**질문**:
1. **피처 수를 줄였는데 성능이 오히려 떨어지는 이유는?** (과적합 완화 vs 정보 손실)
2. **누적 중요도 80%로 선택했는데도 성능이 하락하는 이유는?** (상호작용 피처 손실?)
3. **경기당 알림 수가 2배 이상 증가한 이유는?** (threshold 조정 필요?)
4. 성능이 개선되지 않으면 다음 단계는? (피처 수 조정, 다른 선택 기준?)
5. **원래 53개 피처로 돌아가야 하나, 아니면 다른 선택 기준을 시도해야 하나?**
6. 시퀀스 피처(최근 5초/10초 윈도우)를 더 강화하는 구체적 방법은?
7. GPT 리서치에서 언급한 "시퀀스 피처 다층화"의 구체적 구현 방법은?
